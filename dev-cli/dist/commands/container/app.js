...
class App extends core_1.Command {
    async run() {
        ...
        if (this.argv.length) {
            (0, node_child_process_1.execSync)(`docker exec -it -e XDEBUG_MODE=off -w /var/www buzzingpixel-app bash -c "XDEBUG_MODE=off ${this.argv.join(' ')}";`, { stdio: 'inherit' });
            return;
        }
        this.log(chalk_1.default.yellow("Remember to exit when you're done"));
        (0, node_child_process_1.execSync)('docker exec -it -e XDEBUG_MODE=off -w /var/www buzzingpixel-app bash;', { stdio: 'inherit' });
    }
}
exports.default = App;
