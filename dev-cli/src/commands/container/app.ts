...
export default class App extends Command {
    ...
    public async run (): Promise<void> {
        ...
        if (this.argv.length) {
            execSync(
                `docker exec -it -e XDEBUG_MODE=off -w /var/www buzzingpixel-app bash -c "XDEBUG_MODE=off ${this.argv.join(' ')}";`,
                { stdio: 'inherit' },
            );
        ...
        execSync(
            'docker exec -it -e XDEBUG_MODE=off -w /var/www buzzingpixel-app bash;',
            { stdio: 'inherit' },
        );
    }
}
