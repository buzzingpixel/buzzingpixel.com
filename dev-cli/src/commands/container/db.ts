...
export default class Db extends Command {
    ...
    public static summary = `Execute command in the ${chalk.cyan('db')} container. Empty argument starts a bash session`;
    ...
    public async run (): Promise<void> {
        this.log(chalk.yellow(
            "You're working inside the 'db' application container of this project.",
        ));

        if (this.argv.length) {
            execSync(
                `docker exec -it buzzingpixel-db bash -c "${this.argv.join(' ')}";`,
                { stdio: 'inherit' },
            );
